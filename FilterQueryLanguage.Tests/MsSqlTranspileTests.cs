...
{
    public class MsSqlTranspileTests
    {
        [Fact]
        public void TestParserSimpleExpression()
        {
            var parser = new FQLParser.Parser.FQLTranspiler();
            var options = new FQLTranspilerOptions(TranspilationTarget.MsSql, "(Name notEqual 'Bob')", new List<SyntaxVisitorOption> { new SyntaxVisitorOption { Name = "TableName", Value = "AspNet.Users" } });
        ...
        {
            var parser = new FQLParser.Parser.FQLTranspiler();
            var options = new FQLTranspilerOptions(TranspilationTarget.MsSql, "(Name notEqual 'Bob')  and (Name notEqual 'James') and (Name notEqual 'Brennan')", new List<SyntaxVisitorOption> { new SyntaxVisitorOption { Name = "TableName", Value = "AspNet.Users" } });
            var test = parser.Transpile(options);
        ...
        {
            var parser = new FQLParser.Parser.FQLTranspiler();
            var options = new FQLTranspilerOptions(TranspilationTarget.MsSql, "(((Name notEqual 'Bob')  and (((Name notEqual 'James') or (Name notEqual 'Brennan')))))", new List<SyntaxVisitorOption> { new SyntaxVisitorOption { Name = "TableName", Value = "AspNet.Users" } });
            var test = parser.Transpile(options);
        ...
        {
            var parser = new FQLParser.Parser.FQLTranspiler();
            var options = new FQLTranspilerOptions(TranspilationTarget.MsSql, "((((Name notEqual 'Bob') and (Name notEqual 'Mike'))  and (((Name notEqual 'James') or (Name notEqual 'Brennan')))))", new List<SyntaxVisitorOption> { new SyntaxVisitorOption { Name = "TableName", Value = "AspNet.Users" } });
            var test = parser.Transpile(options);
        ...
        [Fact]
        public void TestMultipleRecursiveStatementsAndManyParanthesis()
        {
            var parser = new FQLParser.Parser.FQLTranspiler();
            var options = new FQLTranspilerOptions(TranspilationTarget.MsSql, "((((Name notEqual 'Bob') and (Name notEqual 'Mike'))  and (((Name notEqual 'James') or ((Name notEqual 'Brennan') and (Name notEqual 'Brennan2'))))))", new List<SyntaxVisitorOption> { new SyntaxVisitorOption { Name = "TableName", Value = "AspNet.Users" } });
            var test = parser.Transpile(options);
            var whatItShouldbe = "SELECT * FROM AspNet.Users WHERE (Name <> 'Bob' and Name <> 'Mike') and (Name <> 'James' or (Name <> 'Brennan' and Name <> 'Brennan2'));";
            Assert.Equal(whatItShouldbe, test);
        }
